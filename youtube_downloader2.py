...
class Ui_YT_Downloader(object):
    ...
    #setting up widget displayed names
    def retranslateUi(self, YT_Downloader):
        ...
        self.Status.setText(_translate("YT_Downloader", "Enter the link above to the YouTube video or playlist you would like to download."))

    #Sets what happens when the download button is clicked
    def Download_Click(self, YT_Downloader):
        
        # takes users inputted youtube video or playlist link
        url = self.URL_Bar.text()
        
        # clears the url bar of text and sets the bottom status label to Downloading
        self.URL_Bar.clear()

        # Bug1
        self.Status.setText('Downloading') # currently a bug here which the label wont update to the new text because the download function overtakes it?
        
        # prevents crashing from non youtube domain
        # Bug2 
        if not url.startswith("https://www.youtube.com/watch?v="):
            self.Status.setText('Please enter youtube, video or playlist, URL')
        else:

            # change url to a list like value
            url = [url]

            #change current working directory to windows downloads directory
            #bug3 only works for windows
            ...
            #download video or playlist from user inputed link     
            ydl = youtube_dl.YoutubeDL({})
            
            # scrape the metadata for the title to indicate finished download in status bar
